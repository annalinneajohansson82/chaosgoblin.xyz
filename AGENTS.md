...
- `blog/_data.yml` — Site-wide data. **This is where custom CSS, fonts, logo, and meta config live.** Keep it as the single source for global styling.
...
1. Edit the `<style>` element in `blog/_data.yml`'s `extra_head`.
2. Re-render and verify in the dev server.
3. If the style block grows past ~50 lines or develops distinct concerns, split into a dedicated CSS file referenced from `extra_head` — but only then.
...
- All custom CSS lives in `blog/_data.yml` until it outgrows that location. Do not scatter it.
- Typography uses Google Sans Flex for display, UI, headings, and body. Google Sans Code for code blocks. These are CDN-loaded, not self-hosted.
