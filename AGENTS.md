...
Before modifying any file, read the relevant section below. Before making design or content decisions, read `docs/PRODUCT.md` (brand, audience, positioning, anti-references) and `docs/DESIGN.md` (typography, color, logo, spacing — concrete values).
...
- **Personal voice** — neurodivergent developer writing about AI, code, and information structure. Not a tutorial mill, not a diary, not a CV page. See `docs/PRODUCT.md` for the full positioning.
...
| CSS | `blog/styles.css`, linked from `extra_head` in `_data.yml` — no SCSS, no build pipeline |
| Hosting | (TBD) |
...
- `blog/styles.css` — Custom CSS. Organized by concern; linked from `extra_head` in `_data.yml`. The **implementation** of the design spec — update both when changing visual values.
- `blog/scripts/nav.js` — Nav interaction behaviors: scroll-shrink, hamburger toggle, overlay open/close, Escape key. Loaded as a module from `base.vto`. Edit here for any nav behavior change.
- `blog/_data.yml` — Site-wide data. Font links, logo, social links, and meta config. Links to `blog/styles.css` via `extra_head`.
...
- `docs/PRODUCT.md` — Brand identity, audience, positioning, anti-references, design principles. **Read this before making design or content decisions.**
- `docs/DESIGN.md` — Concrete design values (typography, color, logo, spacing). **Authoritative for visual design decisions.** Implementation lives in `blog/_data.yml`.
...
2. To include the page in the nav, add to frontmatter:
   ```yaml
   menu:
     visible: true
     order: 0
   ```
   Lower `order` values appear first. This is the **only** mechanism for nav items — do not use `menu_links` in `_data.yml`.
3. Done.

### Adjust global styling
1. Read `docs/DESIGN.md` to confirm the intended value.
2. Edit `blog/styles.css` in the relevant section to match.
3. Update `docs/DESIGN.md` if the value diverges from the spec.
...
- **Do not reintroduce SCSS, UnoCSS, or a local stylesheet pipeline.** The previous attempt was deliberately reset. All custom CSS goes in `blog/styles.css`.
...
- All custom CSS lives in `blog/styles.css`. Do not scatter it. Concrete design values are documented in `docs/DESIGN.md`; keep both in sync.
- Nav items are defined exclusively via page frontmatter (`menu.visible: true`, `menu.order: N`). Do not use `menu_links` in `_data.yml` — it is unused and should not be re-introduced. The logo links to `/`; no separate Home link is needed in the nav.
...
- **Tone in content:** see `docs/PRODUCT.md` — technical-direct-minimal, witty, honest, personal. Not corporate, not cute.
...
## Coding standards
TypeScript/JavaScript: `.agents/skills/standards/references/typescript.md` and `javascript.md`.
All agents touching `.ts` or `.js` files must read the relevant reference first.
...
- `docs/PRODUCT.md` — read before any design or content decision.
- `docs/DESIGN.md` — concrete design values; read before any styling change.
